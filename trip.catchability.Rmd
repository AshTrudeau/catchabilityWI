...
plot(sim.res)
plotResiduals(sim.res, form=data$ln.pop.density)
...
mod.0<-glmmTMB(cpue~1, 
                  data=data, family=glmmTMB::lognormal(link="log"), ziformula=~1)

mod.1<-glmmTMB(cpue~1+(1|wbic) , 
                  data=data, family=glmmTMB::lognormal(link="log"), ziformula=~1+(1|wbic))

mod.2<-glmmTMB(cpue~1+(1|wbic)+(1|year) , 
                  data=data, family=glmmTMB::lognormal(link="log"), ziformula=~1+(1|wbic)+(1|year))
mod.3<-glmmTMB(cpue~1+(1|wbic)+(1|year)+(1|sample.date) , 
                  data=data, family=glmmTMB::lognormal(link="log"), ziformula=~1+(1|wbic)+(1|year)+(1|sample.date))


AICtab(mod.0, mod.1, mod.2, mod.3)
...
```
test those residuals, but may need to look into different visualization. 
...
```{r}
sim<-simulate(mod.pe.month, seed=392)
