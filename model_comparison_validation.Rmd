...
```{r}
data.problem<-data.indexed[c(13, 124, 130, 172),]
```
...
# for order model 2, 1, 6, 7, 3, 8, 4, 5
comp.df$model<-c("all_int","vary_slope", "angler_date","angler_lake",
                 "angler_only","date_lake","date_only","lake_only")
...
```{r}
# much wider prior (sigma~exp(0.25))
fit_prior_check<-stan(file=here("output_selected_model","prior_checks","models_prior_checks","prior_check_all_int_wide_sigma.stan"),
                 data=data.list,
                 #init=init.phi,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
saveRDS(fit_prior_check, here("output_selected_model","prior_checks","models_prior_checks","fits", "prior_check_all_int_int_wide_sigma.rds"))

post.prior.check<-extract(fit_prior_check)

sigma.q.a.post.pc.wide<-post.prior.check$sigma_q_a
sigma.q.d.post.pc.wide<-post.prior.check$sigma_q_d
sigma.q.l.post.pc.wide<-post.prior.check$sigma_q_l


ggplot()+
  geom_density(aes(x=sigma.q.priors), fill="gray")+
  geom_density(aes(x=sigma.q.a.post.pc.wide), fill="lightyellow", alpha=0.5)+
  geom_density(aes(x=sigma.q.d.post.pc.wide), fill="lightgreen", alpha=0.5)+
  geom_density(aes(x=sigma.q.l.post.pc.wide), fill="lightpink", alpha=0.5)+
  ggtitle("sigma.q")+
  theme_bw()
ggsave(here("output_selected_model", "prior_checks", "check_sigma_wider_prior.png"))

# much narrower prior (sigma~exp(10))
fit_prior_check<-stan(file=here("output_selected_model","prior_checks","models_prior_checks","prior_check_all_int_narrow_sigma.stan"),
                 data=data.list,
                 #init=init.phi,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
saveRDS(fit_prior_check, here("output_selected_model","prior_checks","models_prior_checks","fits", "prior_check_all_int_int_narrow_sigma.rds"))

post.prior.check<-extract(fit_prior_check)

sigma.q.a.post.pc.narrow<-post.prior.check$sigma_q_a
sigma.q.d.post.pc.narrow<-post.prior.check$sigma_q_d
sigma.q.l.post.pc.narrow<-post.prior.check$sigma_q_l


ggplot()+
  geom_density(aes(x=sigma.q.priors), fill="gray")+
  geom_density(aes(x=sigma.q.a.post.pc.narrow), fill="lightyellow", alpha=0.5)+
  geom_density(aes(x=sigma.q.d.post.pc.narrow), fill="lightgreen", alpha=0.5)+
  geom_density(aes(x=sigma.q.l.post.pc.narrow), fill="lightpink", alpha=0.5)+
  ggtitle("sigma.q")+
  theme_bw()
ggsave(here("output_selected_model", "prior_checks", "check_sigma_narrow_prior.png"))


```
Beta being the other really important one

```{r}
ggplot()+
  geom_density(aes(x=rlnorm(1000, 0, 2)))
```


```{r}
fit_prior_check<-stan(file=here("output_selected_model","prior_checks","models_prior_checks","prior_check_all_int_wide_beta.stan"),
                 data=data.list,
                 ##init=init.phi,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
saveRDS(fit_prior_check, here("output_selected_model","prior_checks","models_prior_checks","fits", "prior_check_all_int_int_wide_beta.rds"))
post.prior.check<-extract(fit_prior_check)

beta.post.pc.wide<-post.prior.check$beta

ggplot()+
  geom_density(aes(x=beta.prior), fill="gray")+
  geom_density(aes(x=beta.post.pc.wide), fill="lightblue", alpha=0.5)+
  ggtitle("beta")+
  theme_bw()
ggsave(here("output_selected_model", "prior_checks", "check_beta_wider_prior.png"))

fit_prior_check<-stan(file=here("output_selected_model","prior_checks","models_prior_checks","prior_check_all_int_narrow_beta.stan"),
                 data=data.list,
                 ##init=init.phi,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
saveRDS(fit_prior_check, here("output_selected_model","prior_checks","models_prior_checks","fits", "prior_check_all_int_int_narrow_beta.rds"))

post.prior.check<-extract(fit_prior_check)

beta.post.pc.narrow<-post.prior.check$beta

ggplot()+
  geom_density(aes(x=beta.prior), fill="gray")+
  geom_density(aes(x=beta.post.pc.narrow), fill="lightblue", alpha=0.5)+
  ggtitle("beta")+
  theme_bw()
ggsave(here("output_selected_model", "prior_checks", "check_beta_narrower_prior.png"))


```


Parameter distributions in the accepted model are very similar to those returned from a model with a much wider prior on that parameter. Narrower priors, of course, have a strong effect, but that's expected. 


```{r}
fit_selected<-fit_all_int

log_lik_selected<-extract_log_lik(fit_selected, merge_chains=FALSE)
r_eff_selected<-relative_eff(exp(log_lik_selected))

loo_selected<-loo(log_lik_selected, r_eff=r_eff_selected, cores=4)
...
```{r}
mod.summary<-summary(fit_all_int, pars=c("beta","phi","sigma_q_a","sigma_q_d","sigma_q_l",
                                         "log_q_mu", "popDensity"))

mod.summary.comb<-as.data.frame(mod.summary$summary)
write.csv(mod.summary.comb, here("output_selected_model","tables","fit_all_int_param_summary.csv"))
```
