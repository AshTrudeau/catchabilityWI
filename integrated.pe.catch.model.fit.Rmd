...
```{r}
print(real_fit, pars=c("log_q_mu","beta","phi", "mu_q_a", "sigma_q_a", "mu_q_d", "sigma_q_d", "mu_q_l", "sigma_q_l", "popDensity"), probs=c(.025,.5,.975))
...
palette<-c("#78b98f", "#de0ca3", "#74ee65", "#69306e", "#59a20c", "#903be2", "#c0e15c", "#891c1a", "#64d4fd", "#0a4f4e", "#e7ad79", "#1945c5", "#edb1ff", "#683d0d", "#fe707d", "#445a06", "#4787c9", "#fbd127")
...
```
log_q_a might be more distinctive
...
ggplot(log_q_aLong)+
  geom_histogram(aes(x=log_q_a, fill=anglerID))+
...
```
q_d has too many intercepts to show it like this. 
...
ggplot(log_q_lLong)+
  geom_histogram(aes(x=log_q_l, fill=lakeID))+
  geom_vline(xintercept=0, linetype="dashed")+
 # scale_fill_manual(values=palette)+
...
Lots more lake effect here, even when pop density is accounted for
