...
data.list<-list(N=nrow(data.indexed),
                ...
                popDensity_pred=seq(0, 100, by=10)
                )
...
sigma.nb.plot<-ggplot(sigma.nb.df)+
  geom_density(aes(x=value, fill=param), alpha=0.7)+
  scale_fill_manual(values=futurama[5], name="", labels=expression(sigma))+
  xlim(0,3)+
  xlab(expression(sigma))+
  ylab(expression(paste("p(", sigma, "|catch)")))+
  theme_bw()
sigma.nb.plot
...
sigma.plot<-ggplot(sigma.df)+
  ...
  xlim(0,3)+
  ylab(expression(paste("p(", sigma["int"], "|catch)")))+
  xlab(expression(sigma["int"]))+
  theme_bw()
sigma.plot

plot_grid(sigma.nb.plot, sigma.plot, beta.plot, ncol=1, align="v")
ggsave(here("output_selected_model","figures","beta.sigma.posterior.png"), height=6, width=6)
```
Be sure to note that beta is the coefficient for SCALED population density, which makes its scale it more comparable to the sigma values. 
