...
sigma.2.pred.df<-data.frame(param=c(rep("Full model", 8000), rep("Angler effects", 8000), rep("Date effects",8000),
                            ...
                            sigma=c(post_gen$sigma_2_post_full, post_gen$sigma_2_post_angler, post_gen$sigma_2_post_date,
                                    post_gen$sigma_2_post_fixed, post_gen$sigma_2_post_nb_only))%>%

  mutate(param=factor(param, levels=c("No predictors","Angler effects","Date effects","Population density effect","Full model")))


sigma.2.df.sum<-sigma.2.pred.df%>%
  group_by(param)%>%
  summarize(mean.var=mean(sigma))%>%
  ungroup()%>%
  mutate(random.var=rep(mean(post_gen$sigma_2_post_nb_only)),
         mean.var.sub=ifelse(param=="No predictors", mean.var, mean.var-random.var),
...
ggplot(sigma.2.pred.df.lines)+
  geom_density(aes(x=sigma, fill=param))+
  geom_vline(aes(xintercept = mean.var), linetype="dotted")+
  geom_vline(aes(xintercept=mean.var.sub), linetype="dashed")+
  #geom_vline(xintercept=sd(data.list$lmbCatch)^2, linetype="dotted")+
  scale_fill_manual(values=futurama[c(3, 1, 4, 6, 2)], "Model predictions")+
