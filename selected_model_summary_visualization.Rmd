...
#ggsave(here::here("output_selected_model","figures","catch_histogram.png"))

# cpue<-ggplot()+
#   geom_histogram(aes(x=data.list$lmbCatch/exp(data.list$log_effort)), binwidth=1)+
#     xlab("Bass per angler-hour")+
#   ylab("Count")+
#   theme_bw()+
...
effort<-ggplot()+
  geom_histogram(aes(x=exp(data.list$log_effort)), binwidth=0.5)+
...
sigma_2_random <-post_gen$sigma_2_random
sigma_2_fixed<-post_gen$sigma_2_fixed
sigma_2_pred<-post_gen$sigma_2_pred

predictions<-post_gen$posterior_pred_check
predictions.var<-apply(predictions, 1, var)
...
sigma.total.df<-data.frame(param=rep("sigma_total", 8000), sd=post_gen$sigma_total)
...
sigma.error.df<-data.frame(param=rep("sigma_resid", 8000),
                           sd=post_gen$sigma_resid)
...
ggplot()+
  ...
  xlab("Population density (1/ha)")+
   ylab(expression(paste("p(", "Population density", "|Catch)")))+
  theme_bw()
...
ggplot()+
  ...
  ylab(expression(paste("p(", phi, "|Catch)")))+
  xlab(expression(phi))+
  xlim(0,55)+
  theme_bw()
...
```{r}
sigma.check<-data.frame(group=c(rep("prior", 4041), rep("angler", 8000), rep("date", 8000), rep("lake", 8000)),
                        sigma.q=c(sigma.q.priors, sigma.q.a.post, sigma.q.d.post, sigma.q.l.post))

ggplot(sigma.check)+
  geom_density(aes(x=sigma.check$sigma.q, fill=group), alpha=0.5)+
  scale_fill_manual("Effect",values=c("lightpink","lightyellow","lightgreen","gray"))+
  xlim(0,10)+
   ylab(expression(paste("p(", sigma["q"], "|Catch)")))+
...
sigma.pred.df<-data.frame(param=c(rep("Full model", 8000), rep("Angler, date, and lake effects", 8000), 
                                    rep("Population density effect", 8000)),
                            sigma=c(post_gen$sigma_pred, post_gen$sigma_post_random, post_gen$sigma_post_fixed))%>%

  mutate(param=factor(param, levels=c("Angler, date, and lake effects","Population density effect","Full model")))


ggplot(sigma.pred.df)+
  geom_density(aes(x=sigma, fill=param))+
  geom_vline(xintercept=sd(data.list$lmbCatch), linetype="dashed")+
  scale_fill_manual(values=futurama[c(5,6,2)], "Model predictions")+
...
ggsave(here::here("output_selected_model","figures","submodel.sigma.density.png"), height=6, width=6)
```
...
```{r}
 sigma.random.plot<-ggplot(sigma.df)+
   ...
   scale_fill_manual(values=futurama[c(1,4,3)], name="Parameters", labels=expression(sigma[a], sigma[d], sigma[l]))+
   xlim(0,2)+
   ylab(expression(paste("p(", sigma["a,d,l"], "|catch)")))+
   xlab(expression(sigma["a,d,l"]))+
   theme_bw()+
...
# sigma.total.plot
 ...
 # sigma.prediction.plot<-ggplot(sigma.2.pred.df)+
 #   geom_density(aes(x=sigma, fill=param), alpha=0.7)+
 #   #geom_vline(xintercept=sd(data.list$lmbCatch), linetype="dashed")+
 #   scale_fill_manual(values=futurama[2], name="", labels=expression(sigma[hat(lnCatch)]^2))+
 #   xlim(0,2)+
 #   xlab(expression(sigma^2))+
 #   ylab(expression(paste("p(", sigma[hat(lnCatch)]^2, "|Catch)")))+
 #   theme_bw()+
 #   theme(legend.text=element_text(size=16),
 #         axis.title.y=element_text(size=14),
 #         axis.title.x=element_text(size=14))
 ...
 #   geom_density(aes(x=sd^2, fill=param), alpha=0.7)+
...
#   geom_density(aes(x=sd^2, fill=param), alpha=0.7)+
#   scale_fill_manual(values=futurama[6], name="", labels=expression(sigma[beta]^2))+
#   #xlim(0,2)+
#   xlab(expression(sigma[beta]))+
#   ylab(expression(paste("p(", sigma[beta]^2, "|catch)")))+
#   theme_bw()+
#   theme(legend.text=element_text(size=16),
#         axis.title.y=element_text(size=14),
#         axis.title.x=element_blank())
...
# sigma.random.plot<-ggplot(sigma.df)+
#   geom_density(aes(x=sd^2, fill=param), alpha=0.7)+
#   scale_fill_manual(values=futurama[c(1,4,3)], name="", labels=expression(sigma[a]^2, sigma[d]^2, sigma[l]^2))+
#   xlim(0,2)+
#   ylab(expression(paste("p(", sigma["a,d,l"]^2, "|catch)")))+
...
#ggsave(here("output_selected_model","figures","sd.comparison.visual.png"), height=8, width=6)
```
