...
#for each iteration of the model, I subtract the nb_only sub-model variance from the other sub-models (to get unique variance), then divide those by the sum of variance (rather than the variance of the full model predictions)
...
hist(post_gen$prop_var_popDensity_sum)
hist(post_gen$prop_var_angler_sum)
hist(post_gen$prop_var_date_sum)
hist(post_gen$prop_var_nb_sum)

hist(post_gen$prop_var_popDensity_full)
hist(post_gen$prop_var_angler_full)
hist(post_gen$prop_var_date_full)
hist(post_gen$prop_var_nb_full)

# no, this didn't work.

sigma.2.pred.df<-data.frame(param=c(rep("Full model", 8000), rep("Angler effects", 8000), rep("Date effects",8000),
                                     rep("Population density effect", 8000), rep("No covariates", 8000), 
                                    rep("Sum of variance", 8000)),
                             sigma2=c(post_gen$sigma_2_post_full, post_gen$sigma_2_post_angler, post_gen$sigma_2_post_date,
                                     post_gen$sigma_2_post_fixed, post_gen$sigma_2_post_nb_only, post_gen$sum_sigma_2))%>%
 
   mutate(param=factor(param, levels=c("No covariates","Angler effects","Date effects","Population density effect","Full model", "Sum of variance")))

prop.var.df<-data.frame(param=c(rep("Angler effects", 8000), rep("Date effects", 8000), 
                                rep("Population density effect", 8000), rep("No covariates", 8000)),
                        prop.var=c(post_gen$prop_var_angler, post_gen$prop_var_date, post_gen$prop_var_popDensity,
 ...
 sigma.2.pred.wide<-data.frame(sigma2_full=post_gen$sigma_2_post_full,
                              ...
                              sigma2_PE_nb=post_gen$sigma_2_post_fixed,
                              sigma2_nb=post_gen$sigma_2_post_nb_only,
                              sum_sigma2=post_gen$sum_sigma_2,
                              prop_angler=post_gen$prop_var_angler,
                              prop_date=post_gen$prop_var_date,
                              prop_popDensity=post_gen$prop_var_popDensity,
                              prop_random=post_gen$prop_var_nb)%>%
   mutate(sigma2_angler=sigma2_angler_nb-sigma2_nb,
          sigma2_date=sigma2_date_nb-sigma2_nb,
          sigma2_popDensity=sigma2_PE_nb-sigma2_nb)
...
I'm so lost. Left off here 12/27/24. Struggling with proportion variance still--was adding up the variance of each submodel really appropriate? I'm counting the random variation multiple times! 
```{r}
