...
pacman::p_load(tidyverse, here, lubridate, lme4, loo, rstan, shinystan, truncnorm, MFEUtilities, RColorBrewer, cowplot, bayesplot, cmdstanr, posterior, ggsci, cowplot, geosphere)
```
...
sigma.2.pred.df<-data.frame(param=c(rep("Full model", 8000), rep("Angler effects", 8000), rep("Date effects",8000),
                                    rep("Population density\neffect", 8000), rep("No covariates", 8000)),
  ...
  mutate(param=factor(param, levels=c("No covariates","Angler effects","Date effects","Population density\neffect","Full model")))
...
sigma.2.df.sum<-sigma.2.pred.df%>%
  ...
  mutate(random.var=rep(mean(post_gen$sigma_2_post_nb_only)),
         mean.var.sub=ifelse(param=="No covariates", mean.var, mean.var-random.var),
