...
data.list<-list(N=nrow(data.indexed),
                ...
                z_scores = z_scores,
                z_scores_long= rep(z_scores, each=11),
                log_popDensity_pred=log(seq(1, 101, by=10))
                )
...
sigma.total.plot<-ggplot(sigma.total.df)+
  ...
  scale_fill_manual(values=futurama[5], name="", labels=expression(sigma["total"]))+
  xlim(0,7)+
...
sigma.fixed.plot<-ggplot(sigma.fixed.df)+
  geom_density(aes(x=sd, fill=param), alpha=0.7)+
  scale_fill_manual(values=futurama[5], name="", labels=expression(sigma[beta]))+
  xlim(0,7)+
...
```{r}
quant.catch.df<-as.data.frame(post_gen$predict_quantile_angler_catch)
...
ggplot(quant.catch.long)+
  geom_violin(aes(x=`Angler skill`, y=`Mean catch`), fill=futurama[7], adjust=2)+
  ...
  coord_flip()+
  xlab("Angler catch power percentile")+
  ylab("Predicted mean catch")+
  theme_bw()
