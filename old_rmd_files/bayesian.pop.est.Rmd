...
write.csv(fish_pe_recap, here::here("output","recap.sumctmt.sumrt.surfaceArea.csv"))
...
print(pe_all_fit, pars=c("PE","popDensity"), probs=c(.025,.5,.975))

traceplot(pe_all_fit, pars=c("PE","popDensity"), inc_warmup=FALSE, nrow=2)

draws<-extract(pe_all_fit)

pe.draws.df<-as.data.frame(draws$PE)
popDensity.draws.df<-as.data.frame(draws$popDensity)

lakeID<-fish_pe_recap$lakeID

names(pe.draws.df)<-c(lakeID)
names(popDensity.draws.df)<-lakeID

popDensity.draws.long.lognorm<-popDensity.draws.df%>%
  dplyr::select(AR:WS)%>%
  pivot_longer(cols=everything(), names_to="lakeID", values_to="popDensity")

ggplot(popDensity.draws.long.lognorm)+
  geom_histogram(aes(x=popDensity, fill=lakeID))

mean.sd.popDensity<-popDensity.draws.long.lognorm%>%
  group_by(lakeID)%>%
  summarize(mean.popDensity=mean(popDensity),
            sd.popDensity=sd(popDensity))

write.csv(mean.sd.popDensity, here::here("output","bayesian.pop.density.csv"))
```
...
palette<-c(brewer.pal(n=12, "Paired"), "gray")

ggplot(pe.draws.long.gamma)+
  geom_histogram(aes(x=PE, fill=lakeID))+
  facet_wrap(lakeID~., ncol=3)+
  scale_fill_manual(values=palette)+
  theme_bw()
ggsave(here::here("figures","population.estimates.png"), height=4, width=6)
