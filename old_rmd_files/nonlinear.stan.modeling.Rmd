...
fake_fit_hier<-stan(file="noncentered.hierarchical.nonlinear.nb.stan",
                 data=fake.data.hier,
                 init=hier.inits,
...
bh.inits1 <- function() {
  a_tilde = rnorm(nLake, 0, 1)
  b_tilde = rnorm(nLake, 0, 1)
  mu_a = rnorm(1, 100, 0.5)
  mu_b = rnorm(1, 100, 0.5)
  sigma_a_unif = runif(1,-1,1)
  sigma_b_unif = runif(1,-1,1)
  sigma = runif(1,0.1,0.5)
  init <- list(a_tilde=a_tilde, b_tilde=b_tilde, 
               sigma=sigma, sigma_a_unif=sigma_a_unif, sigma_b_unif=sigma_b_unif, 
               mu_a=mu_a, mu_b=mu_b)
  return(init)
}
...
fake_fit_centered_hier<-stan(file="fake.hierarchical.nonlinear.nb.stan",
                 data=fake.data.hier,
                 #init=hier.inits,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
launch_shinystan(fake_fit_centered_hier)

```
trying linearized version noncentered
...
fake.data.hier<-list(N=nrow(fake.df.hier),
                     ...
                     log_popDensity=log(fake.df.hier$popDensity),
                     lmbCatch=fake.df.hier$lmbCatch)



fake_fit_noncentered_linearized<-stan(file="noncentered.hierarchical.LINEARIZED.stan",
                 data=fake.data.hier,
                 init=hier.inits,
                 control=list(stepsize=0.1, max_treedepth=15),
                 chains=4,
                 ...
                 cores=4,
                 refresh=0)
launch_shinystan(fake_fit_noncentered_linearized)


```

This model also runs with lmbCatch as a Poisson distribution