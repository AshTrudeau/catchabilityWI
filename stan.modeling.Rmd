...
# sd.error<-sqrt(5)/1
# 
# ln.pe.sd<-sd.error/PE



cpue<-cpue.q*PE.error^cpue.beta+rnorm(1000, mean=0, sd=1)

lncpue<-log(cpue.q*PE.error^cpue.beta+rnorm(1000, mean=0, sd=1))

lncpue<-log(cpue.q)+PE.error
...
catchSomething<-rbinom(n=1000, size=1, prob=catchSomethingProb)
  

simple.fake.data.error<-data.frame(PE=PE.error,
                                   pe_se=error,
                                   cpue=cpue,
                                   catchSomething=catchSomething)
...
fake.df.error<-list(N=length(simple.fake.data.error$PE),
                    PE=simple.fake.data.error$PE,
                    se_pe=rep(10, 1000),
...
simple.fake.data.no0<-data.frame(lnPE=log(PE),
                                   cpue=cpue)

linear.data<-list(N=length(simple.fake.data.no0$lnPE),
                  lnPE=simple.fake.data.no0$lnPE,
                  cpue=simple.fake.data.no0$cpue)


fit.linear<-stan(
  file="fake.log.linear.stan",
  data=linear.data,
  chains=4, 
  warmup=1000, 
  iter=2000,
...
print(fit.linear, pars=c("lnq", "beta", "lp__"), probs=c(.1,.5,.9))

traceplot(fit.linear, pars=c("lnq","beta"), inc_warmup=FALSE, nrow=2)
...
```
  That worked great; now linear with error in PE
