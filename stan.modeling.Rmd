...
catch<-effort*catch.q*PE.error^catch.beta*rlnorm(250, 0, 0.1)
...
simple.fake.data<-list(popDensity=PE.error,
                             fcatch=catch,
                             effort=effort,
                             pe_sd=rep(5, length(PE.error)),
                             N=length(PE.error))


fit.linear<-stan(
  file="fake.log.linear.error.catch.stan",
  data=simple.fake.data,
  chains=4, 
  warmup=1000, 
  iter=2000,
  cores=4, 
