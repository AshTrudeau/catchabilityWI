...
// For model comparison, this is model with only 1 mean q term
...
for(i in 1:N){
  ...
  logCatchHat[i] = log_effort[i] + log_q_mu + log_q + beta * log_popDensity_sc[LL[i]];
}
...
generated quantities{
  ...
  for(n in 1:N){
    posterior_pred_check[n]=neg_binomial_2_log_rng(log_effort[n] + log_q + beta * log_popDensity_sc[LL[n]],phi);
  }
  
  for(i in 1:N){
    log_lik[i] = neg_binomial_2_log_lpmf(lmbCatch[i]|log_effort[i] + log_q + beta * log_popDensity_sc[LL[i]], phi);
  }
}
